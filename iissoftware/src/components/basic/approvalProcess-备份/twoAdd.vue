...
export default {
    ...
    methods: {
        ...
        nodeClick(row) {
            let pid = this.$store.state.approvalProcessStore.id,
        ...
        save() {
            if(this.currentNode['parentId'] == 0) {         //如果没有上级部门，也就是上级是公司了，那么parentName就是公司名称
                ...
                this.$store.commit('approvalProcessStore/updateNodes', {level: 2, node: this.currentNode});        //保存后，将当前选中的会计科目节点保存到store里面
                this.close();
            } else {
