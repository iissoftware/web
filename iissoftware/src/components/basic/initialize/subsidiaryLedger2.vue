        ...
        <div class="subsidiaryLedger">
            ...
            <div class="block-area">
                <div class="btn-box">
                    ...
                    <!-- 表格数据 -->
                <div class="table">
                    <el-table :data="subsidiaryRow" border v-if="$store.state.initializeVal[0]['name'] == '应收账款'" @selection-change="handleSelectionChange" @row-contextmenu="rowContextmenu" class="tb-edit" highlight-current-row>
                        ...
                        <el-table-column property="type" label="类别">
                            <template slot-scope="scope">
                                <el-select v-model="scope.row.type" ref="MA28"  @change="getSelectObj" placeholder="选择销售方式">
                                    <el-option :label="categoryDataKH[0]['name']" :value="categoryDataKH[0]['id']"></el-option>
                                </el-select>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column property="compactCode" label="销售合同编号">
                            <template slot-scope="scope">
                                <el-select v-model="scope.row.compactCode" ref="MA28" v-if='scope.row.name'>
                                    <el-option v-for="(items, index) in xsData" :label="items.givenNumberOne" :value="items.id" :key="index"></el-option>
                                </el-select>
                                <el-select v-model="scope.row.xsnull" ref="MA28" v-else>
                                    <el-option v-for="(items, index) in xsceshi" :key="index"></el-option>
                                </el-select>
                            </template>
                        </el-table-column>
    ...
    export default {
        ...
        data() {
            return {
                ...
                selectRowId:[],
                categoryDataKH: this.$store.state.initializeStore.categoryDataKH,                       //类别
                ...
                xsData:this.$store.state.initializeStore.xsData,            //销售合同编号数据
                radioState: '',                     //启用系统状态
                xsceshi:[],                         //当没有填名称的时候，销售合同编号显示空
            }
        },
        watch: {
            //监听往来对象名称弹窗选中的名称
        ...
        methods: {
            ...
            addRow(){                               //新增
                this.subsidiaryRow.push({type:this.categoryDataKH[0]['name'],name:'',compactCode:'',money:'',number:''})
            },
            ...
            exportExal(){                           //导出
                if(this.multipleSelection.length > 0) {
                    ...
                    this.$http.get(this.$url + 'betweenObject/export',{params: { list: idSting }}).then(res => {
                        this.$message({message: '导出成功', type: 'success'});
                        window.location.href = this.$url + 'betweenObject/export?list=' + idSting;          //导出时触发
                    }).catch(err => {
        ...
        mounted() {
            ...
            this.$store.dispatch('initializeStore/getSubsidiaryData', this.$url + 'betweenObject/selectObjectById?subejctId='+this.$store.state.initializeVal[0]['id']);
            //获取销售合同编号所有数据
            this.$store.dispatch('initializeStore/getXsSubClassDetails', {url: this.$url + 'auxiliaryInfoAllCategory/selectContractNumber', id: this.$store.state.initializeStore.xsCompactData[0]['id'], categoryId: this.$store.state.initializeStore.xsCompactData[0]['dataCategoryId']});
            setTimeout(() => {
                //获取类别数据
