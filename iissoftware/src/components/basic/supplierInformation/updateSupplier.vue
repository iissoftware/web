<template>
    ...
    <div class="addRowBlock">
        <!-- 前翻后翻组件 -->
        <modify :tableData="$store.state.supplierStore.tableData" :currentId="rowId" @getPositionData="getPositionData"/>
        <!-- 表单数据 -->
        <el-form label-width="140px" :model="addRowData" ref="addRowForm" status-icon :rules="addFormRules" style="margin-top:10px">
...
export default {
    ...
    data(){
        ...
        return{
            ...
            departmentDrog: '',                     //存储分管部门名字
            rowId: this.$store.state.supplierStore.rowId,                  //当前修改数据的id
    ...
    methods:{
        ...
        getPositionData(row) {                  //前翻后翻
            this.addRowData = row;
            this.departmentDrog = this.addRowData['divisionDepartment'];
            if(this.addRowData['provinceId'] || this.addRowData['countryId']){
                this.$http.get(this.$url + "city/findProvince?parentId=" + this.addRowData['countryId']).then(res => {
                    if(res.data.code == 20001) {
                        this.locationProvinceList = res.data.data;
                        if(this.addRowData['cityId'] || this.addRowData['provinceId']){
                            this.$http.get(this.$url + "city/findCity?parentId=" + this.addRowData['provinceId']).then(res => {
                                if(res.data.code == 20001) {
                                    this.locationCityList = res.data.data;
                                }
                            })
        ...
        getUpdateData(){                        //获取当前修改数据
            this.$http.get(this.$url + 'supplier/selectInfoById?id=' + this.rowId).then(res => {
                if(res.data.code == 20001) {
                    this.addRowData = res.data.data;
                    this.departmentDrog = this.addRowData['divisionDepartment']
                    if(this.addRowData['provinceId'] || this.addRowData['countryId']){
                        this.$http.get(this.$url + "city/findProvince?parentId=" + this.addRowData['countryId']).then(res => {
                            if(res.data.code == 20001) {
            ...
            // for(var i = 0 ; i< this.$store.state.supplierStore.tableData.length; i++){
            //     if(this.$store.state.supplierStore.tableData[i]['id'] == this.rowId){
            //         this.index = i;
            //     }
            // }
        },
