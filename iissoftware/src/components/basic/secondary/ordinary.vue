<template>
    <div class="form">
        <el-form :model="formData" status-icon ref="formRef" :rules="formRules" label-width="100px" class="demo-ruleForm" style="overflow: hidden;">
            <el-form-item label="名称：" prop="detailedName" style="margin-top: 10px !important;">
                <el-input type="text" v-model="formData.detailedName" @keyup.enter.native="enterKey($event, 'DPT1')" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="备注：" prop="remarks">
                <el-input type="text" v-model="formData.remarks" autocomplete="off" ref="DPT1" @keyup.enter.native="save('formRef')"></el-input>
            </el-form-item>
...
export default {
    data() {
        return {
            ...
            continueAdd: false,
            formRules: {
                detailedName: [{required: true, message: '名称不能为空', triggle: 'blur'}]
            },
    ...
    methods: {
        getUpdateData() {           //获取更新数据
            this.$http.get(this.$url + 'auxiliaryInfoAllCategory/selectByOrdinaryId?id=' + this.rowId).then(res => {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    if(!this.formData['id']) {          //新增保存
                        this.formData['auxiliaryInfoId'] = row['id'];           //小类id
                        this.$http({
                            url: this.$url + 'auxiliaryInfoAllCategory/insertOrdinary',
                            method: 'post',
                        ...
                        this.$http({
                            url: this.$url + 'auxiliaryInfoAllCategory/updateOrdinary',
                            method: 'post',
                            params: this.formData
                        }).then(res => {
                            if(res.data.code == 20001) {
