                ...
                <el-form :model="formData" ref="formRef" label-width="120px" :rules="formRules" style="overflow: hidden;">
                    ...
                    <el-form-item label="科目类别：" prop="type">
                        <el-select v-model="formData.type" placeholder="选择科目类别" style="width: 100%;" ref="MA3" @keyup.enter.native="enterKey($event, 'MA4')" @change="selectChange">
                            <el-option v-for="item in subjectTypes" :key="item['id']" :label="item['name']" :value="item['id']" v-show="item['id'] > 8"></el-option>
                        </el-select>
                    </el-form-item>
...
<<<<<<< HEAD
                            <el-option label="不核算" :value="0" :label="'不核算'"></el-option>
=======
                            <el-option label="不核算" value="0"></el-option>
>>>>>>> 412dd94f16eb89dc6aa9dab511721bd5f5bd12c5
...
export default {
    ...
    data() {
        return {
            ...
            selectTypeId: '',
            rowId: this.$store.state.subjectStore.rowId,
    ...
    methods: {
        getUpdateData() {
            this.$http.get(this.$url + 'subjectGroup/selectGroupById?id=' + this.rowId).then(res => {
                if(res.data.code == 20001) {
                    if(res.data.data) {
                        res.data.data['journal'] = Boolean(res.data.data['journal']);
                        this.formData = res.data.data;
                        this.betweenObject = Boolean(res.data.data['betweenObject']);
                        this.$store.commit('subjectStore/updateBetweenObject', res.data.data['betweenObject']);
                        this.$store.commit('subjectStore/updateObjectList', res.data.data['objectList']);
                    }
                }
            });
        },
